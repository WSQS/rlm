...
class AgentExecutor:
    def __init__(self, ic: "ReplInstance"):
        self.ic = ic
        self.client = Anthropic()

    def execute(self, system: str, messages: list[MessageParam]) -> AgentResult:
        ...
        while True:
            ...
            for block in message.content:
                if block.type == "thinking":
                    ...
                    has_tool = True
                    print(f"Tool:\n{block}")
                    r = self.ic.run(str(block.input["code"]))
            ...
            if self.ic.final_result:
                return AgentResult(
                    final_answer=self.ic.final_result, conversation=conversation
                )
        ...
        return AgentResult(final_answer=None, conversation=conversation)
...
class ReplInstance:
    def __init__(self):
        ...
        def AGENT(system: str, messages: list[MessageParam]) -> AgentResult:
            executor = AgentExecutor(self)
            return executor.execute(system, messages)
